...
async function ExchangeRateExist(BaseCountryId, CurrencyId) {
  ...
  try {
    ...
    console.error(
      "[DB] Error checking exchange rate existence:",
      error.message
    );
  ...
  try {
    ...
    if (exists) {
      return NextResponse.json(
        {
          message:
            "Exchange rate for this Base Country and Currency already exists",
        },
        { status: 409 }
      );
    }

    const data = await CreateExchangeRate(
      BaseCountryId,
      CurrencyId,
      ExchangeRate
    );
...
export async function PUT(req) {
  try {
    ...
    console.log(error);
    return NextResponse.json(
      { message: "Cannot update exchange rate" },
      { status: 500 }
    );
  }
}
